...
class NeuralNetwork:
    def __init__(self, layer_dims, activation_funcs, keep_probs=None, lambd=0):
        self.layers = []
        self.lambd = lambd  # L2 regularization lambda
        if keep_probs is None:
            keep_probs = [1.0] * len(layer_dims)  # Default no dropout
        for i in range(1, len(layer_dims)):
            keep_prob = keep_probs[i-1] if i < len(keep_probs) else 1.0
    ...
    def train(self, X, Y, learning_rate=0.0075, num_iterations=3000):
        for i in range(num_iterations):
            AL = self.forward_propagation(X)
            cost = self.compute_cost(AL, Y)
            self.backward_propagation(Y, AL)
